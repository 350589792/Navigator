...
class HybridModel:
    def __init__(self, ai_model, cfd_model, config):
        """
        初始化混合模型
        Args:
            ai_model: 训练好的AI模型
        ...
        # 融合权重
        self.ai_weight = config.get('ai_weight', 0.6)
        self.cfd_weight = config.get('cfd_weight', 0.4)
    ...
    def predict(self, input_data, return_details=False):
        ...
        try:
            # 1. AI模型预测
            ai_pred, ai_attention = self.ai_model.predict(input_data)
    ...
    def calculate_ai_confidence(self, ai_pred):
        """计算AI预测的置信度"""
        # 基于预测方差计算置信度
        _, lower, upper = self.ai_model.calculate_prediction_interval(ai_pred)
    ...
    def calculate_local_weights(self, ai_confidence, cfd_confidence):
        ...
        weights = {
            'ai': (ai_confidence / total_confidence) * self.ai_weight,
            'cfd': (cfd_confidence / total_confidence) * self.cfd_weight
        }
    ...
    def save_model(self, path):
        """保存模型"""
        model_state = {
            'ai_model_state': self.ai_model.state_dict(),
            'config': self.config,
            'weights': {
                'ai': self.ai_weight,
                'cfd': self.cfd_weight
    ...
    def load_model(self, path):
        ...
        self.ai_model.load_state_dict(model_state['ai_model_state'])
        self.config = model_state['config']
        self.ai_weight = model_state['weights']['ai']
        self.cfd_weight = model_state['weights']['cfd']