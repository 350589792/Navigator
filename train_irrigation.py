...
def train_model(
    ...
    """Train the irrigation classification model and save metrics.
    ...
    labels = []
    for _, _, _, irr_label in train_loader:
        labels.extend(irr_label.numpy())
    ...
    for epoch in range(num_epochs):
        ...
        for images, texture_features, _, labels in train_loader:
        ...
        with torch.no_grad():
            for images, texture_features, _, labels in val_loader:
                images = images.to(device)
                texture_features = texture_features.to(device)
                labels = labels.to(device)
...
def evaluate_model(
    ...
    with torch.no_grad():
        for images, texture_features, _, labels in test_loader:
            images = images.to(device)
            texture_features = texture_features.to(device)
            labels = labels.to(device)
    ...
    with open(os.path.join(output_dir, 'classification_report.txt'), 'w') as f:
        f.write("Irrigation Classification Report:\n")
        f.write(report_str)
...
def main():
    ...
    output_dir = f'outputs/irrigation_classification_{timestamp}'
