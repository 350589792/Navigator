...
from tensorflow.keras.layers import Input, Conv1D, Concatenate, Dense, LSTM, Bidirectional
...
# 定义 MSC-Transformer-LSTM 模型
def build_msc_transformer_lstm_model(input_shape):
    ...
    # Transformer编码器
    transformer_output = transformer_encoder(merged, num_heads=4, key_dim=16, ff_dim=64)

    # LSTM
    lstm_output = Bidirectional(LSTM(32, return_sequences=True))(transformer_output)
...
for noise_type in noise_types:
    ...
    # 构建模型
    model = build_msc_transformer_lstm_model(input_shape=x_train.shape[1:])
    model_name = f"MSC_Transformer_LSTM_{noise_type}"
