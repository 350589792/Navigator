...
from tensorflow.keras.layers import Conv1D, InputLayer, Bidirectional, LSTM, Dense
...
def build_cnn_bilstm_model(input_shape):
    model = Sequential([
        ...
        Conv1D(16, 3, padding="same", activation="relu"),
        Conv1D(16, 3, padding="same", activation="relu"),
        Bidirectional(LSTM(16, return_sequences=True)),  # 双向 LSTM 层
        Dense(1, activation="linear")  # 输出一维数据
    ])
...
for noise_type in noise_types:
    ...
    # 构建模型
    model = build_cnn_bilstm_model(input_shape=x_train.shape[1:])
    model_name = f"CNN_BiLSTM_{noise_type}"
